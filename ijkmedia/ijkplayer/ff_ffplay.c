...
static ijk_sei_callback sei_callback;
void ffp_global_set_sei_callback(ijk_sei_callback cb)
{
    sei_callback = cb;
}

 static ijk_render_callback render_callback;
...
void mem2hex(const char *pData, const unsigned uiDataLen, bool bCap) {
    char pBuf[3] = {0};
    ...
    char* strResult = (char*)av_mallocz(uiDataLen);
...
//        strcat(strResult + (i*2), pBuf);
//        strncat(strResult + (i*2), pBuf, 2);
        printf("%c%c", pBuf[0], pBuf[1]);
    }
    printf("\n");
//    return strResult;
    av_free(strResult);
...
//    while(n < pkt->size) {
//        uint32_t naluLen =
//        (pkt->data[n+0] & 0xff << 24) +
//        (pkt->data[n+1] & 0xff << 16) +
//        (pkt->data[n+2] & 0xff << 8) +
//        (pkt->data[n+3] & 0xff << 0);
//
//        Byte nalu_predata[4] = {pkt->data[n+0], pkt->data[n+1], pkt->data[n+2],pkt->data[n+3]};
//        Byte annex_b_nalu_header[4] = {0, 0, 0, 1};
//
//        if (!memcmp(nalu_predata, annex_b_nalu_header, sizeof(annex_b_nalu_header))) {
//
//        if ((pkt->data[n+4] & 0x1f) == 0x06 || (pkt->data[n+4] & 0x7e) >> 1 == 39) {
//        // pkt->data[ 0 1 2 3]`code
//        // pkt->data[4] == 0x06  SEI帧
//        // pkt->data[5...] == 0x05  SEI payload type = user_data_unregistered
//        // pkt->data[...] SEI payload size 不包含payload size部分: https://www.itu.int/rec/dologin_pub.asp?lang=e&id=T-REC-H.264-200305-S!!PDF-E&type=items
//
//            n += ((pkt->data[n+4] & 0x7e) >> 1 == 39 ? 6 : 5);
//            int j = n;
//            // int j = n + ((pkt->data[n+4] & 0x7e) >> 1 == 39 ? 6 : 5);
//            int playloadType = 0;
//            int payloadSize = 0;
//            do {
//                playloadType += pkt->data[j];
//            } while(pkt->data[j++] == 0xff);
//
//
//            if (playloadType == 0x05) { //SEI payload type = user_data_unregistered
//                payloadSize = 0;
//                do {
//                    payloadSize += pkt->data[j];
//                } while(pkt->data[j++] == 0xff);
//                payloadSize += 1;  //the '0x80'
//
//
//                //防冲突处理
//                char* rbsp_buf = (char*) av_mallocz(payloadSize);
//                int ii = 0;
//                int startIndex = j;
//                while(j < payloadSize + startIndex) {
//                    if(j + 3 < payloadSize &&
//                        pkt->data[j] == 0x00 && pkt->data[j+1] == 0x00 && pkt->data[j+2] == 0x03 &&
//                        pkt->data[j+3] < 0x04) {
//                        rbsp_buf[ii++] = 0x00;;
//                        rbsp_buf[ii++] = 0x00;
//                        rbsp_buf[ii++] = pkt->data[j+3];
//                        j += 4;
//                    } else {
//                        rbsp_buf[ii++] = pkt->data[j++];
//                    }
//                }
//
//                char tmps[17] = {0};
//                memcpy(tmps, rbsp_buf, 16);
//                printf("parsing sei:%s\n", tmps);
//
//
//                if (strncmp(rbsp_buf, "TencentStreamSEI", 16) == 0) {
//                }
//
//
//                //判断uuid
//                if (strncmp(rbsp_buf, "AdsCtrlSEIV00001", 16) == 0) {
//                    int method = (int)(rbsp_buf[16]);
//                    char* json = rbsp_buf + 17;
//                    // 找到并去除结尾符 '\x80'
//                    j = 0;
//                    while(json[j] && json[j] != '\x80') j++;
//                    if (json[j] == '\x80') json[j] = 0;
//
//                    if (sei_callback) {
//                        (*sei_callback)(pkt->pts, method, json);
//                    }
//                }
//                av_free(rbsp_buf);
//            }
...
//            // SEI帧解析方法
//            if (pkt->stream_index == is->video_stream && pkt->data[4] == 0x06) {
//                int n = 0;
//                while(n <  pkt->size) {
//                    unsigned char nalu_predata[4] = {pkt->data[n+0], pkt->data[n+1], pkt->data[n+2],pkt->data[n+3]};
//                    unsigned char annex_b_nalu_header[4] = {0, 0, 0, 1};
//                    unsigned char annex_b_nalu_header2[3] = {0, 0, 1};
//
//                    if (memcmp(nalu_predata, annex_b_nalu_header, sizeof(annex_b_nalu_header)) ||
//                        memcmp(nalu_predata, annex_b_nalu_header2, sizeof(annex_b_nalu_header2))) {
//
////                    if (memcmp(nalu_predata, annex_b_nalu_header, sizeof(annex_b_nalu_header)) &&
////                        ((pkt->data[n+4] & 0x1f) == 0x06 || (pkt->data[n+4] & 0x7e) >> 1 == 39)) {
//
//                        bool hevc = (pkt->data[n+4] & 0x7e) >> 1 == 39;
//                        n += hevc ? 6 : 5;
//                        int j = n;
//                        int playloadType = 0;
//                        int payloadSize = 0;
               ...
               while(n < pkt->size) {
                   ...
                   if ((pkt->data[n+4] & 0x1f) == 0x06 || (pkt->data[n+4] & 0x7e) >> 1 == 39) {
                       ...
                       if (playloadType == 0x05) { //SEI payload type = user_data_unregistered
                           ...
                           memcpy(tmps, rbsp_buf, 16);
                           //判断uuid
                           if (strncmp(rbsp_buf, "AdsCtrlSEIV00003", 16) == 0) {
                               int method = (int)(rbsp_buf[16]);
                               char* json = rbsp_buf + 17;
                               // 找到并去除结尾符 '\x80'
                               j = 0;
